...
export default function ScheduleModal({
        ...
        if (show) {
            if (ScheduleToEdit) {
                setData({
                    ...
                    check_out_time: ScheduleToEdit.check_out_time.substring(
                        0,
                        5,
                    ),
                    entry_tolerance_minutes:
                        ScheduleToEdit.entry_tolerance_minutes,
                    exit_tolerance_minutes:
                        ScheduleToEdit.exit_tolerance_minutes,
    ...
    const submit: FormEventHandler = (e) => {
        ...
        if (ScheduleToEdit) {
            put(`/horarios/${ScheduleToEdit.id}`, options);
        } else {
            post('/horarios', options);
        }
    };
