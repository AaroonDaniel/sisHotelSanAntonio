...
// Interfaz para la lista REAL de ocupados
...
interface Props {
    ...
    services: Service[];
    activeCheckins: ActiveCheckin[]; // Recibimos la lista correcta
}
...
export default function CheckinDetailsIndex({
    ...
    checkinDetails,
    services,
    activeCheckins, // <--- ESTO ES LO QUE NECESITAMOS
}: Props) {
    ...
    // --- CORRECCIÓN: Usamos la lista directa del controlador ---
    // Antes usabas 'uniqueCheckins' derivado de los detalles. Eso estaba mal.
    // Ahora usamos 'activeCheckins' que trae las 3 habitaciones ocupadas.
    const listForModal = activeCheckins || []; 

    const handleAddService = (checkinId: number | null) => {
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div>
                    ...
                    <p className="mt-1 text-gray-400">
                        Vista detallada de los consumos y servicios en ocupaciones.
                    </p>
                </div>
                                            ...
                                            <tr key={detail.id} className="transition-colors hover:bg-gray-50">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2 font-bold text-gray-900">
                                                        ...
                                                        {detail.checkin?.room?.number || 'S/N'}
                                                    </div>
                                                    <span className="ml-7 text-xs text-gray-500">
                                                        {detail.checkin?.room?.room_type?.name}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="font-medium text-gray-800">
                                                            {detail.checkin?.guest?.full_name || 'Desconocido'}
                                                        </span>
                                                    </div>
                                                    <div className="ml-6 text-xs text-gray-500">
                                                        CI: {detail.checkin?.guest?.identification_number || '-'}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <div className="font-medium text-gray-900">
                                                        {detail.service?.name || 'Servicio General'}
                                                    </div>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-1 text-xs text-green-700">
                                                        ...
                                                        {formatDate(detail.checkin?.check_in_date)}
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                                                        !detail.checkin?.check_out_date ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                                                    }`}>
                                                        {!detail.checkin?.check_out_date ? 'Ocupado' : 'Finalizado'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex justify-end gap-2">
                                                        {!detail.checkin?.check_out_date && (
                                                            <button
                                                                onClick={() => detail.checkin && handleAddService(detail.checkin.id)}
                                                        ...
                                                        <button className="text-gray-400 transition hover:text-blue-600" title="Ver Detalle">
                                                            <FileText className="h-4 w-4" />
                                                        </button>
                                                        <button
                                                            onClick={() => {
                                                                if (detail.checkin_id) {
                                                                    window.open(`/checks/${detail.checkin_id}/receipt`, '_blank');
                                                                }
                                                            }}
                ...
                <ServiceModal
                    ...
                    // --- CORRECCIÓN FINAL: Pasamos la lista completa de ocupados ---
                    availableCheckins={listForModal}
                />
            </div>
        </AuthenticatedLayout>
    );
