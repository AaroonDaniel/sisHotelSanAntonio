...
import { ArrowLeft, Pencil, Plus, Search, Trash2, CheckCircle, AlertTriangle, Eye, Wrench } from 'lucide-react';
...
export default function MaintenancesIndex({ auth, maintenances, rooms }: any) {
    ...
    // Estados de modales
    const [selectedMaintenance, setSelectedMaintenance] = useState<any>(null); // Para finalizar
    const [modalData, setModalData] = useState<{ show: boolean, data: any }>({ show: false, data: null }); // Crear/Editar
    const [deleteId, setDeleteId] = useState<number | null>(null); // Eliminar

    // --- LÓGICA DE FILTRADO ---
    // Filtra por el motivo del daño o por el número de habitación
    const filteredMaintenances = maintenances.data.filter((m: any) =>
                ...
                <div className="py-8">
                    <div className="mx-auto w-full overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
                        ...
                        <div className="flex flex-col items-start justify-between gap-4 border-b border-gray-200 bg-white p-6 sm:flex-row sm:items-center">
                            ...
                            <button
                                onClick={() => setModalData({ show: true, data: null })}
                            ...
                            <table className="w-full text-left text-sm text-gray-600">
                                <thead className="bg-gray-50 text-xs text-gray-700 uppercase">
                                    <tr>
                                        ...
                                        <th className="px-6 py-4 font-bold">Falla / Motivo</th>
                                        <th className="px-6 py-4 font-bold">Fecha Reporte</th>
                                    ...
                                    {filteredMaintenances.length > 0 ? (
                                        filteredMaintenances.map((m: any) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    <p className="text-xs text-gray-500 truncate max-w-[200px]">{m.description || 'Sin detalles'}</p>
                                                </td>
                                                
                                                <td className="px-6 py-4">
                                                    <span className="block font-medium">{formatDate(m.created_at)}</span>
                                                    <span className="block text-xs text-gray-400">Por: {m.user?.name}</span>
                                                </td>
                                                ...
                                                <td className="px-6 py-4">
                                                    ...
                                                    >
                                                        {m.resolved_at ? <CheckCircle className="w-3 h-3" /> : <AlertTriangle className="w-3 h-3" />}
                                                        {m.resolved_at ? 'Reparado' : 'En Mantenimiento'}
                                                    </span>
                                                </td>
                                                ...
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex justify-end items-center gap-3">
                                                        ...
                                                        {/* Botón Editar */}
                                                        <button
                                                            onClick={() => setModalData({ show: true, data: m })}
                                                            title="Editar"
                                        ...
                                        <tr>
                                            <td colSpan={5} className="p-8 text-center text-gray-500">
                                                {searchTerm ? 'No se encontraron reportes con ese término.' : 'No hay registros de mantenimiento aún.'}
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
