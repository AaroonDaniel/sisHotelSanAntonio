...
import { ArrowLeft, BedDouble, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function LodgingControlHistory({ date }: Props) {
    ...
    return (
        <AuthenticatedLayout user={auth.user}>
            ...
            <div className="mx-auto flex h-[calc(100vh-4rem)] max-w-6xl flex-col px-4 py-6 sm:px-6 lg:px-8">
                <button
                    onClick={() => router.visit('/status')}
                    className="group mb-4 flex w-fit items-center gap-2 text-sm font-medium text-gray-400 transition-colors hover:text-white"
                >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-400">
                                Revisá el checklist de cualquier día anterior
                            </p>
                        </div>
                    </div>
                ...
                {/* Visor PDF */}
                <div className="min-h-0 flex-1 rounded-2xl border border-gray-700 bg-gray-300/50 p-2">
                    <iframe
                        key={src}
                        src={src}
