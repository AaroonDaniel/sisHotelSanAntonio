...
export default function DeleteModal({ show, onClose, roleId }: DeleteModalProps) {
    const { delete: destroy, processing } = useForm();

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        if (roleId) {
            destroy(`/roles/${roleId}`, {
                preserveScroll: true,
                onSuccess: () => onClose(),
            });
        }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm transition-opacity animate-in fade-in duration-200">
            <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl animate-in zoom-in-95 duration-200">
                <div className="p-6 text-center">
                    ...
                    <h3 className="mb-2 text-xl font-bold text-gray-800">¿Eliminar Cargo?</h3>
                    <p className="text-gray-500">
                        Esta acción no se puede deshacer. Los usuarios asignados a este cargo perderán sus accesos.
                    </p>
