...
export default function ReservationModal({
    ...
    const filteredGuests = useMemo(() => {
        ...
        return guests.filter((g) => {
            ...
            return fullName.toLowerCase().includes(guestQuery.toLowerCase()) || ci.includes(guestQuery);
        });
    }, [guests, guestQuery]);
    ...
    const submit: FormEventHandler = (e) => {
        ...
        if (data.payment_type === 'QR' && data.advance_payment > 0 && !data.qr_bank) {
            alert("Por favor selecciona un banco para el pago QR.");
            return;
        }
        const options = {
            preserveScroll: true,
                        ...
                        <div className="flex h-full flex-col space-y-1 lg:col-span-8">
                                ...
                                {data.details.map((detail, index) => (
                                    <div key={index} className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-shadow hover:shadow-md">
                                        <div className="flex items-center gap-4">
                                            <div className="flex h-14 w-14 flex-col items-center justify-center rounded-xl border border-blue-100 bg-blue-50 text-blue-700 shrink-0">
                                                <span className="text-xl leading-none font-bold">{detail._temp_pax_count}</span>
                                                <span className="text-[9px] font-bold uppercase">PERS</span>
                                            </div>
                                            <div className="grid flex-1 grid-cols-1 md:grid-cols-3 gap-3">
                                                ...
                                                <div>
                                                    <label className="mb-1 block text-[10px] font-bold text-gray-400 uppercase">Precio Noche</label>
                                                    <select value={detail.price_id} onChange={(e) => updateDetailRow(index, 'price_id', e.target.value)} disabled={!detail.room_id} className="w-full rounded-lg border-gray-300 py-1.5 text-xs font-bold uppercase text-gray-700 focus:border-blue-500 focus:ring-blue-500 disabled:bg-gray-100">
                                                        <option value="">...</option>
                                                        {/* @ts-ignore */}
                                                        {rooms.find(r => r.id.toString() === detail.room_id)?.prices?.filter((p: any) => p.bathroom_type === detail._temp_bathroom_filter).map((p: any) => (<option key={p.id} value={p.id}>{p.amount} Bs</option>))}
                                                    </select>
                                                </div>
                                            </div>
                                            <div className="w-20 text-right shrink-0">
                                                <label className="mb-1 block text-[10px] font-bold text-gray-400 uppercase">Total</label>
                                                <div className="text-sm font-bold text-gray-800">{detail.price} Bs</div>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="mt-auto rounded-2xl border border-gray-200 bg-gray-50 p-5">
                                    ...
                                    <div className="flex-1 flex flex-col gap-2 relative -top-3 -mt-4 animate-in duration-300 fade-in slide-in-from-top-2 pt-6">
                                        ...
                                        <div className={`border-t border-gray-100 pt-1.5 transition-all duration-300 ${data.payment_type === 'QR' ? 'visible opacity-100' : 'invisible opacity-0 h-0 overflow-hidden'}`}>
                                            ...
                                            <div className="grid grid-cols-4 gap-1">
                                                {[{ id: 'YAPE', label: 'YAPE', color: 'border-green-200 bg-green-50 text-green-700' }, { id: 'FIE', label: 'FIE', color: 'border-orange-200 bg-orange-50 text-orange-700' }, { id: 'BNB', label: 'BNB', color: 'border-blue-200 bg-blue-50 text-blue-700' }, { id: 'ECO', label: 'ECO', color: 'border-yellow-200 bg-yellow-50 text-yellow-700' }].map((banco) => (
                                ...
                                <div className="mt-4 border-t border-gray-200 pt-4">
                                    ...
                                    <textarea rows={1} value={data.observation} onChange={(e) => setData('observation', e.target.value)} className="w-full rounded-lg border-gray-300 text-sm focus:border-gray-500 focus:ring-gray-500" placeholder="Detalles adicionales..." />
                                </div>
                            </div>
                        </div>
                    </div>
