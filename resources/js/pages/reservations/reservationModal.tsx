...
const isRoomAvailable = (status?: string) => {
    ...
    return s === 'LIBRE' || s === 'AVAILABLE' || s === 'DISPONIBLE';
};
        ...
        if (show) {
            if (reservationToEdit) {
                const mappedDetails: DetailItem[] = (reservationToEdit.details || []).map((det: any) => {
                    const priceObj = det.price_relation || (typeof det.price === 'object' ? det.price : null);
                ...
                setData((prev) => ({
                    ...
                    arrival_date: new Date().toISOString().split('T')[0],
                    details: [
                        { room_id: '', price_id: '', price: 0, _temp_pax_count: 1, _temp_bathroom_filter: '', _temp_room_type_id: '' },
                    ],
                }));
            }
            clearErrors();
    ...
    const handleGuestCountChange = (newCount: number) => {
        ...
        setData((prev) => {
            ...
            for (let i = 0; i < newBreaks.length; i++) {
                if (newBreaks[i]) { groups.push(currentGroupSize); currentGroupSize = 1; } 
    ...
    const updateDetailRow = (index: number, field: keyof DetailItem, value: any) => {
        ...
        if (field === '_temp_bathroom_filter') {
            ...
            if (room) {
                ...
                if (typeId) {
                    newDetails[index]._temp_room_type_id = typeId.toString();
                }
            } else {
                newDetails[index].price_id = '';
                newDetails[index].price = 0;
                newDetails[index]._temp_bathroom_filter = '';
                newDetails[index]._temp_room_type_id = '';
            }
        }
            ...
            <div className="flex max-h-[80vh] w-full max-w-6xl animate-in flex-col overflow-hidden rounded-2xl bg-white shadow-2xl duration-200 zoom-in-95">
                ...
                <form id="reservation-form" onSubmit={submit} className="flex-1 overflow-y-auto bg-white p-6">
                                    ...
                                    {isGuestDropdownOpen && filteredGuests.length > 0 && (
                                        <div className="absolute z-50 mt-1 max-h-48 w-full overflow-y-auto rounded-xl border border-gray-400 bg-white shadow-xl">
                                            {filteredGuests.map((g) => (
                                                <div key={g.id} onClick={() => selectGuest(g)} className="cursor-pointer border-b border-gray-50 px-4 py-3 text-sm transition-colors last:border-0 hover:bg-green-50">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="date" disabled value={data.arrival_date} className="w-full cursor-not-allowed rounded-xl border border-gray-400 bg-gray-100 py-2 pl-10 text-sm text-gray-600 focus:ring-0" />
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="relative">
                                            ...
                                            <input type="time" value={data.arrival_time} onChange={(e) => setData('arrival_time', e.target.value)} className="w-full rounded-xl border border-gray-400 py-2 pl-10 text-sm text-black focus:border-green-500 focus:ring-green-500" />
                                        </div>
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                                    ...
                                                    <div key={`row-${blockIndex}`} className="flex w-full animate-in flex-col items-center fade-in slide-in-from-left-2">
                                                        {blockIndex > 0 && (
                                                            <div className="relative my-0.5 flex h-4 w-0.5 items-center justify-center bg-gray-300">
                                                                <button type="button" onClick={() => toggleBreak(block[0] - 1)} className="absolute z-10 rounded-full border border-gray-300 bg-white p-0.5 shadow-sm hover:border-green-500 hover:text-green-600"><LinkIcon className="h-2 w-2" /></button>
                                                            </div>
                                                        )}
                                                ...
                                                <div key="grid-last" className="flex w-full flex-col">
                                                    {renderedElements.length > 0 && (
                                                        <div className="relative my-0.5 flex h-4 w-full justify-center">
                                                            ...
                                                            <button type="button" onClick={() => toggleBreak(accumulatedSingles[0][0] - 1)} className="absolute top-1/2 z-10 -translate-y-1/2 rounded-full border border-gray-300 bg-white p-0.5 shadow-sm hover:border-green-500 hover:text-green-600"><LinkIcon className="h-2 w-2" /></button>
                                                        </div>
                                                    )}
                        ...
                        <div className="flex h-full flex-col space-y-1 lg:col-span-8">
                            ...
                            <div className="max-h-[380px] flex-1 space-y-4 overflow-y-auto p-1">
                                {data.details.map((detail, index) => {
                                    ...
                                    // 1. Filtro Libre de Tipos de Habitación (Muestra todos a menos que se haya filtrado el baño)
                                    const validTypesMap = new Map();
                                    rooms.forEach(r => {
                                        const isStatusValid = isRoomAvailable(r.status) || r.id.toString() === detail.room_id;
                                        if(!isStatusValid) return;

                                        const p = getExactRoomPrice(r);
                                        const matchesBathroom = detail._temp_bathroom_filter ? (p && isMatchingBathroom(p.bathroom_type, detail._temp_bathroom_filter)) : true;
                                        
                                        if (matchesBathroom) {
                                            const typeId = r.room_type?.id || r.room_type_id;
                                            const typeName = r.room_type?.name || 'Tipo ' + typeId;
                                            if (typeId) {
                                                validTypesMap.set(typeId, { id: typeId, name: typeName });
                                            }
                                        }
                                    });
                                    const availableRoomTypes = Array.from(validTypesMap.values());

                                    // 2. Filtro Libre de Habitaciones (Muestra las 60 a menos que el usuario filtre primero)
                                    const availableRooms = rooms.filter(r => {
                                        const isStatusValid = isRoomAvailable(r.status) || r.id.toString() === detail.room_id;
                                        
                                        const typeId = r.room_type?.id?.toString() || r.room_type_id?.toString();
                                        const isTypeValid = detail._temp_room_type_id ? typeId === detail._temp_room_type_id : true;
                                        
                                        const p = getExactRoomPrice(r);
                                        const hasCorrectBathroom = detail._temp_bathroom_filter ? (p ? isMatchingBathroom(p.bathroom_type, detail._temp_bathroom_filter) : false) : true;
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <select 
                                                            ...
                                                            className="w-full rounded-lg border-gray-300 py-1.5 text-xs font-bold uppercase text-gray-700 focus:border-blue-500 focus:ring-blue-500"
                                                        >
                                                    ...
                                                    {/* 3. HABITACIÓN - AHORA TOTALMENTE LIBRE */}
                                                    <div>
                                                        ...
                                                        <select 
                                                            ...
                                                            className="w-full rounded-lg border-gray-300 py-1.5 text-xs font-bold uppercase text-gray-700 focus:border-blue-500 focus:ring-blue-500"
                                                        >
                                                            <option value="">Elegir Hab...</option>
                            ...
                            <div className="mt-auto rounded-2xl border border-gray-200 bg-gray-50 p-5">
                                <div className="flex flex-col gap-6 md:flex-row">
                                    <div className="relative -top-3 -mt-4 flex flex-1 animate-in flex-col gap-2 pt-6 duration-300 fade-in slide-in-from-top-2">
                                        <div className="flex gap-2">
                                            <div className="w-1/2">
                                                ...
                                                <div className="flex rounded-lg bg-gray-100 p-0.5">
                                                    <button type="button" onClick={() => setData((prev) => ({ ...prev, payment_type: 'EFECTIVO', qr_bank: '' }))} className={`flex-1 rounded py-1 text-[9px] font-bold transition-all ${data.payment_type === 'EFECTIVO' ? 'bg-white text-green-700 shadow-sm ring-1 ring-gray-200' : 'text-gray-400 hover:text-gray-600'}`}>EFECTIVO</button>
                                                    <button type="button" onClick={() => setData('payment_type', 'QR')} className={`flex-1 rounded py-1 text-[9px] font-bold transition-all ${data.payment_type === 'QR' ? 'bg-white text-purple-700 shadow-sm ring-1 ring-gray-200' : 'text-gray-400 hover:text-gray-600'}`}>QR</button>
                                                </div>
                                            </div>
                                            <div className="w-1/2">
                                        ...
                                        <div className={`border-t border-gray-100 pt-1.5 transition-all duration-300 ${data.payment_type === 'QR' ? 'visible opacity-100' : 'invisible h-0 overflow-hidden opacity-0'}`}>
                                            ...
                                            <div className="grid grid-cols-4 gap-1">
                                                {[
                                                    ...
                                                    <button key={banco.id} type="button" onClick={() => setData('qr_bank', banco.id)} className={`rounded border px-0.5 py-1 text-[8px] font-bold transition-all active:scale-95 ${data.qr_bank === banco.id ? `ring-1 ring-purple-500 ring-offset-0 ${banco.color} scale-105 shadow-sm brightness-95` : `border-gray-200 bg-white text-gray-500 hover:border-gray-300 hover:bg-gray-50`}`}>{banco.label}</button>
                                                ))}
                                            </div>
                                            {data.advance_payment > 0 && !data.qr_bank && (
                                                <div className="mt-1 flex animate-pulse items-center justify-center gap-0.5 text-[8px] font-bold text-red-500">
                                                    <AlertCircle className="h-2 w-2" /><span>Seleccione Banco</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="mt-6 flex justify-end gap-3 border-t border-gray-100 pt-4">
                        ...
                        <button type="submit" disabled={processing} className="flex items-center gap-2 rounded-xl bg-green-600 px-4 py-2 text-sm font-bold text-white shadow-md transition hover:bg-green-500 active:scale-95 disabled:opacity-50">
                            {processing ? 'Guardando...' : <><Save className="h-4 w-4" /> {reservationToEdit ? 'Actualizar' : 'Guardar Reserva'}</>}
                        </button>
                    </div>
                </form>
            </div>
