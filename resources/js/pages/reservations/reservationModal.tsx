...
export default function ReservationModal({
        ...
        if (show) {
            if (reservationToEdit) {
                const mappedDetails: DetailItem[] = (
                    ...
                    const priceObj =
                        det.price_relation ||
                        (typeof det.price === 'object' ? det.price : null);
    ...
    const handleGuestCountChange = (newCount: number) => {
        ...
        setData((prev) => {
            ...
            // 🚀 NUEVO: Buscamos si ya existe algún baño filtrado en el formulario
            // para unificarlo si entra en modo corporativo
            const forcedBath = isCorporate
                ? prev.details.find((d) => d._temp_bathroom_filter)
                      ?._temp_bathroom_filter || ''
                : '';
            ...
            const newDetails: DetailItem[] = groups.map((groupSize, index) => {
                ...
                // Declaramos las demás variables para poder limpiarlas si el baño no coincide
                let newBath = existing ? existing._temp_bathroom_filter : '';
                let newRoomId = existing ? existing.room_id : '';
                let newPriceId = existing ? existing.price_id : '';
                let newRoomTypeId = existing ? existing._temp_room_type_id : '';

                // Si es tarifa corporativa y tenemos un baño base, lo forzamos a esta fila
                if (isCorporate && forcedBath) {
                    newBath = forcedBath;
                }

                if (existing && existing.room_id) {
                    const room = rooms.find(
                        (r) => r.id.toString() === existing.room_id,
                    );
                    const p = room ? getExactRoomPrice(room) : null;

                    if (p) {
                        // 🚀 NUEVO: Si al volverse corporativa, esta habitación no cumple
                        // con el baño forzado, la limpiamos.
                        if (
                            isCorporate &&
                            forcedBath &&
                            !isMatchingBathroom(p.bathroom_type, forcedBath)
                        ) {
                            newRoomId = '';
                            newPriceId = '';
                            newPrice = 0;
                            ...
                            if (isCorporate) {
                                ...
                                const isPrivate =
                                    p.bathroom_type.toLowerCase() ===
                                        'private' ||
                                    p.bathroom_type.toLowerCase() === 'privado';
                ...
                return {
                    ...
                    _temp_bathroom_filter: newBath,
                    _temp_room_type_id: newRoomTypeId,
    ...
    const toggleBreak = (index: number) => {
        ...
        setData((prev) => {
            ...
            // =======================================================
            // 🚀 VERIFICAR SI ES TARIFA CORPORATIVA
            // =======================================================
            ...
            const newDetails: DetailItem[] = groups.map((groupSize, idx) => {
                ...
                // Si ya hay un cuarto asignado y es tarifa corporativa, actualizamos su precio total
                // basado en el nuevo tamaño del grupo (camas).
                if (existing && existing.room_id && isCorporate) {
                    const room = rooms.find(
                        (r) => r.id.toString() === existing.room_id,
                    );
                    const p = room ? getExactRoomPrice(room) : null;
                    if (p) {
                        const isPrivate =
                            p.bathroom_type.toLowerCase() === 'private' ||
                            p.bathroom_type.toLowerCase() === 'privado';
                        const rate = isPrivate ? 90 : 60; // 90 Privado, 60 Compartido
                        newPrice = rate * groupSize;
                    }
                }
    ...
    const updateDetailRow = (
        ...
        // Evaluamos si es tarifa corporativa para usarlo en toda la función
        ...
        if (field === '_temp_bathroom_filter') {
            if (isCorporate) {
                // 🚀 REGLA CORPORATIVA: Forzar el mismo baño a TODAS las habitaciones
                newDetails.forEach((det, i) => {
                    det._temp_bathroom_filter = value;

                    // Si la fila ya tenía un cuarto asignado, verificar si sigue cumpliendo la regla
                    if (det.room_id) {
                        const room = rooms.find(
                            (r) => r.id.toString() === det.room_id,
                        );
                        const p = room ? getExactRoomPrice(room) : null;
                        if (p && !isMatchingBathroom(p.bathroom_type, value)) {
                            // Si no coincide con el nuevo baño corporativo, limpiar la selección
                            det.room_id = '';
                            det.price_id = '';
                            det.price = 0;
                            det._temp_room_type_id = '';
                        }
                    } else if (i === index) {
                        // Limpiar campos dependientes solo de la fila actual si no tenía cuarto
                        det._temp_room_type_id = '';
                        det.room_id = '';
                        det.price_id = '';
                        det.price = 0;
                ...
                // Comportamiento normal (no corporativo)
            ...
            if (room) {
                ...
                if (p) {
                    ...
                    // Rellena el Baño automáticamente de la habitación seleccionada
                    const newBath = p.bathroom_type.toLowerCase();
                    newDetails[index]._temp_bathroom_filter = newBath;
                    ...
                    if (isCorporate) {
                        const isPrivate =
                            newBath === 'private' || newBath === 'privado';
                        ...
                        // 🚀 SINCRONIZAR BAÑO: Aplicar este mismo baño a las demás filas
                        newDetails.forEach((det, i) => {
                            if (i !== index) {
                                det._temp_bathroom_filter = newBath;
                                // Limpiar si el cuarto que tenían ya no coincide
                                if (det.room_id) {
                                    const r2 = rooms.find(
                                        (r) => r.id.toString() === det.room_id,
                                    );
                                    const p2 = r2
                                        ? getExactRoomPrice(r2)
                                        : null;
                                    if (
                                        p2 &&
                                        !isMatchingBathroom(
                                        ...
                                        // Actualizar precio de la otra fila si es corporativa y sí coincide
                                        det.price =
                                            ratePerPerson *
                                                                            ...
                                                                            {localIdx <
                                                                                ...
                                                                                <div className="mx-1">
                                                                                    <button
                                                                                        ...
                                                                                        className="flex h-5 w-5 items-center justify-center rounded-full border border-blue-200 bg-blue-50 text-green-400 transition-all hover:border-red-300 hover:bg-red-50 hover:text-red-500"
                                                                                    >
                        ...
                        <div className="flex h-full flex-col space-y-1 lg:col-span-8">
                            ...
                            <div className="max-h-[380px] flex-1 space-y-4 overflow-y-auto p-1">
                                {data.details.map((detail, index) => {
                                    // 🚀 NUEVO: Obtener todas las habitaciones seleccionadas EXCEPTO la de esta fila
                                    const selectedRoomIds = data.details
                                        .map((d, i) => (i !== index ? d.room_id : ''))
                                        .filter((id) => id !== '');
                                    ...
                                    rooms.forEach((r) => {
                                        const isStatusValid =
                                            isRoomAvailable(r.status) ||
                                            r.id.toString() === detail.room_id;
                                        const isAlreadySelected = selectedRoomIds.includes(r.id.toString());
                                        
                                        // 🚀 Excluimos si no es válida o si ya fue seleccionada en otra fila
                                        if (!isStatusValid || isAlreadySelected) return;
                                    ...
                                    const availableRooms = rooms.filter((r) => {
                                        ...
                                        const isTypeValid =
                                            detail._temp_room_type_id
                                                ? typeId ===
                                                  detail._temp_room_type_id
                                                : true;

                                        const p = getExactRoomPrice(r);
                                        const hasCorrectBathroom =
                                            detail._temp_bathroom_filter
                                                ? p
                                                    ? isMatchingBathroom(
                                                          p.bathroom_type,
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="relative">
                                                            ...
                                                            <input
                                                                ...
                                                                className="w-full cursor-not-allowed rounded-lg border-gray-200 bg-green-50 py-1.5 pl-8 text-xs font-bold text-green-700 focus:ring-0"
                                                                placeholder="0.00"
                                                            />
                                                        </div>
                                                    </div>
