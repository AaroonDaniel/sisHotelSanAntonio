...
export default function ToleranceModal({
    show,
    onClose,
    type,
    data,
    ...
    const config = {
        ...
        denied: {
            ...
            // Entrada: la ventana se define en minutos antes de la hora
            // oficial, así que ese número es lo relevante. Salida: la
            // ventana es una hora límite fija (checkout + tolerancia), no
            // tiene sentido mostrar "minutes" ahí (antes siempre llegaba
            // en 0 porque ese caso nunca lo llenaba).
            message: isEntry
                ? `No es posible aplicar la tolerancia. La hora actual excede el límite de ${data.minutes} minutos permitidos.`
                : `No es posible aplicar la tolerancia. La hora actual ya superó el límite máximo de salida (${data.time}).`,
        },
    };
    ...
    return (
        <div className="fixed inset-0 z-[100] flex animate-in items-center justify-center bg-black/60 p-4 backdrop-blur-sm duration-200 fade-in">
            <div className="w-full max-w-sm animate-in overflow-hidden rounded-2xl bg-white shadow-2xl zoom-in-95">
                ...
                <div className="p-6 text-center">
                    <div
                        className={`mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full ${current.colorBg} ${current.colorText}`}
                    >
                        {current.icon}
                    </div>
